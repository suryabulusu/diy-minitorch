...
def central_difference(f, *vals, arg=0, epsilon=1e-6):
    ...
    def get_new_arg(x):
        return vals[:arg] + [x] + vals[arg + 1:]

    return (f(*get_new_arg(vals[arg] + epsilon)) - f(*get_new_arg(vals[arg] - epsilon))) / (2 * epsilon)
...
class Scalar(Variable):
    ...
    def __add__(self, b):
        ...
        return Add.apply(self, b)  # returns a scalar, wrapped by Add function which is defined below
        # the add function is special -- because it has derivative too
